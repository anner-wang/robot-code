...
class Robot:
    ...
    def move(self, motion, tolerance = 0.001, max_steering_angle=pi/4.0):
        """
        move robot with streering and motion command
        """
        steering = motion[0]
        distance = motion[1]
    ...
    def measurement_prob(self, measurements):
        ...
        for i in range(len(measurements)):
            ...
            # update Gaussian
            prob *= (exp(- (error_bearing ** 2) / (self.bearing_noise ** 2) / 2.0) /  
...
def generate_ground_truth(motions):
    """
    generates the measurements vector. You can use it to develop your solution.
    """    
    myrobot = Robot()
...
def check_output(final_robot, estimated_position):
    """
    checks to see if your particle filter localizes the robot to within the desired 
